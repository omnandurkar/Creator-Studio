...
import { publishedReleases, type Release } from "@/lib/content";
...
function ReleaseCard({ release, index }: { release: Release; index: number }) {
  ...
  return <Link className={`music-release-card music-release-card--${index % 4}`} href={`/music/${release.slug}`}><div className="music-release-card__art">{release.coverImage ? <img alt={`${release.title} cover artwork`} src={release.coverImage} /> : <ReleaseArt index={index} />}<span className="music-release-card__index">{String(index + 1).padStart(2, "0")}</span>{release.isIllustrative && <span className="music-release-card__sample">first proof</span>}<span className="music-release-card__play"><Play fill="currentColor" size={16} /></span></div><div className="music-release-card__details"><div><p>{release.type}</p><span>{year}</span></div><h2>{release.title}</h2>{release.mood?.[0] && <span className="music-release-card__mood">{release.mood[0]}</span>}<span className="music-release-card__open">open release <ArrowUpRight size={15} /></span></div></Link>;
}
...
export default function Music() {
  ...
  return <SiteShell pageTheme="music"><section className="music-hero"><div className="music-hero__grid" aria-hidden="true" /><div className="music-hero__copy"><p className="music-hero__eyebrow"><span /> the blue room / vol. 02</p><h1>For the songs<br />that need <em>space</em><br />around them.</h1><p>Singles, albums, late-night demos, and collaborations from Om’s evolving sound world.</p><a className="music-hero__jump" href="#release-shelf"><ArrowDown size={17} /> Browse the shelf</a></div><div className="music-hero__art" aria-label="Abstract record and equalizer illustration"><div className="music-hero__equalizer" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /><i /></div><div className="music-hero__record"><span /><i /></div><button aria-expanded={isSleeveFlipped} aria-label="Turn the music sleeve for a production note" className={`music-hero__sleeve ${isSleeveFlipped ? "is-flipped" : ""}`} onClick={() => setIsSleeveFlipped((flipped) => !flipped)} type="button"><span>{isSleeveFlipped ? "keep the\nfirst strange\ntake." : "Om\nNandurkar"}</span><i /><i /><i /></button><div className="music-hero__ring music-hero__ring--one" /><div className="music-hero__ring music-hero__ring--two" /><p>turn it up,<br />then look closer.</p></div><div className="music-hero__serial">SIDE A · 00—∞</div></section>
